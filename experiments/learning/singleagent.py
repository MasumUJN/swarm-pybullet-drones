...
def run(
    ...
    """
    Runs training for an rl singleagent

    Parameters
    ----------
    env : _type_, optional
        _description_, by default DEFAULT_ENV
    algo : _type_, optional
        _description_, by default DEFAULT_ALGO
    obs : _type_, optional
        _description_, by default DEFAULT_OBS
    act : _type_, optional
        _description_, by default DEFAULT_ACT
    cpu : _type_, optional
        _description_, by default DEFAULT_CPU
    steps : _type_, optional
        _description_, by default DEFAULT_STEPS
    ...
    sa_env_kwargs = dict(
        aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS, obs=obs, act=act
    )
    ...
    model.learn(
        total_timesteps=steps, callback=eval_callback, log_interval=100,  # int(1e12),
    )
